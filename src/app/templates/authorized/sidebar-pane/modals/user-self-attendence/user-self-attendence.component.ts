...
@Component({
  selector: 'app-user-self-attendence',
  templateUrl: './user-self-attendence.component.html',
  styleUrls: ['./user-self-attendence.component.scss'],
  providers: [FormatTimestampPipe],
})
export class UserSelfAttendenceComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.subscriptionObj =
      this.eventEmitterService.selfAttendenceModalEventEmitter.subscribe(
        (status) => {
          console.log(status);
    ...
    let lat_in = localStorage.getItem('lat_in');
    if (lat_in != null) {
      this.latIn = Number(lat_in);
    }
    let lon_in = localStorage.getItem('lon_in');
    if (lon_in != null) {
    ...
    let lat_out = localStorage.getItem('lat_out');
    if (lat_out != null) {
      this.latOut = Number(lat_out);
    }
    let lon_out = localStorage.getItem('lat_out');
    if (lon_out != null) {
  ...
  openUserAttendenceModal() {
    ...
    let add_in = localStorage.getItem('address_in');
    if (add_in != null) {
      this.addIn = add_in;
    }
    ...
    let add_out = localStorage.getItem('address_out');
    if (add_out != null) {
      this.addOut = add_out;
    }
